...
def cal_rpn(imgsize,featuresize,scale,gtboxes):
    ...
    gt_argmax_overlaps = overlaps.argmax(axis=0)  #按列方向寻找最大值的索引
    
 
    #the anchor with the highest IOU overlap with a GT box
    anchor_argmax_overlaps = overlaps.argmax(axis=1) #按行方向寻找最大值的索引
...
def gen_sample(xmlpath,imgpath,batchsize=1):
    ...
    while 1:
        ...
        # img = cv2.imread(imgpath+ "\\"+ imgfile, 0)
        # h,w= img.shape
        ...
        # print(m_img.shape)
        # m_img = np.expand_dims(m_img,axis=3)
        # print(m_img.shape)
...
def testrpn():
    ...
    xmlpath = 'Annotations\img_4375.xml'
    imgpath = 'JPEGImages\img_4375.jpg'
    gtbox,_ = readxml(xmlpath)
    img = cv2.imread(imgpath)
    h,w,c= img.shape
    [cls,regr],base_anchor = cal_rpn((h,w),(int(h/16),int(w/16)),16,gtbox)
    
    temp_base_anchor = base_anchor.astype(int)
    for i in temp_base_anchor:
    ...
    regr = np.expand_dims(regr,axis=0)  #添加一列
